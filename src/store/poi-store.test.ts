...
describe('POI Store', () => {
  ...
  describe('Initial State', () => {
    ...
    it('should have some POIs not revealed', () => {
      ...
      expect(hiddenPOIs.map(poi => poi.slug)).toEqual(['central-park', 'times-square']);
    });
  });
  ...
  describe('revealLocation', () => {
    it('should reveal a POI by slug', () => {
      ...
      expect(usePOIStore.getState().pois.find(poi => poi.slug === 'central-park')?.isRevealed).toBe(false);
    ...
    it('should not affect other POIs when revealing one', () => {
      ...
      const initialTimesSquare = initialState.pois.find(poi => poi.slug === 'times-square');
      const initialBrooklynBridge = initialState.pois.find(poi => poi.slug === 'brooklyn-bridge');
      ...
      const brooklynBridge = state.pois.find(poi => poi.slug === 'brooklyn-bridge');
      
      // Other POIs should maintain their original state
      expect(timesSquare?.isRevealed).toBe(initialTimesSquare?.isRevealed);
      expect(brooklynBridge?.isRevealed).toBe(initialBrooklynBridge?.isRevealed);
    });
    ...
    it('should handle revealing a POI that is already revealed', () => {
      ...
      expect(usePOIStore.getState().pois.find(poi => poi.slug === 'brooklyn-bridge')?.isRevealed).toBe(true);
    ...
    it('should maintain POI properties when revealing', () => {
      ...
      const initialCentralPark = usePOIStore.getState().pois.find(poi => poi.slug === 'central-park');
      
      revealLocation('central-park');
      
      const revealedCentralPark = usePOIStore.getState().pois.find(poi => poi.slug === 'central-park');
      ...
      expect(revealedCentralPark?.longitude).toBe(initialCentralPark?.longitude);
      expect(revealedCentralPark?.description).toBe(initialCentralPark?.description);
  ...
  describe('resetLastRevealedPOI', () => {
    ...
    it('should not affect POIs when resetting lastRevealedPOISlug', () => {
      ...
      expect(stateAfterReset.pois.find(poi => poi.slug === 'central-park')?.isRevealed).toBe(true);
    });
  ...
  describe('reset', () => {
    it('should reset the store to initial state', () => {
      ...
      expect(modifiedState.pois.find(poi => poi.slug === 'central-park')?.isRevealed).toBe(true);
      expect(modifiedState.pois.find(poi => poi.slug === 'times-square')?.isRevealed).toBe(true);
    ...
    it('should reset POIs to their initial revealed states', () => {
      ...
      expect(resetState.pois.find(poi => poi.slug === 'central-park')?.isRevealed).toBe(false);
      expect(resetState.pois.find(poi => poi.slug === 'times-square')?.isRevealed).toBe(false);
      expect(resetState.pois.find(poi => poi.slug === 'brooklyn-bridge')?.isRevealed).toBe(true);
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle concurrent reveal operations', () => {
      ...
      expect(state.pois.find(poi => poi.slug === 'central-park')?.isRevealed).toBe(true);
      expect(state.pois.find(poi => poi.slug === 'times-square')?.isRevealed).toBe(true);
      expect(state.pois.find(poi => poi.slug === 'brooklyn-bridge')?.isRevealed).toBe(true);
